    ...
    <div className="space-y-6">
      ...
      {isAdding && (
        <form onSubmit={handleAdd} className="bg-[#111c2d] rounded-2xl p-6 border border-white/5 flex flex-wrap gap-4">
          <input value={newUser.name} onChange={(e) => setNewUser({ ...newUser, name: e.target.value })} placeholder="الاسم" className="flex-1 min-w-[150px] bg-[#0b1422] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-cyan-400/50" required />
          <input value={newUser.email} onChange={(e) => setNewUser({ ...newUser, email: e.target.value })} placeholder="البريد" type="email" className="flex-1 min-w-[150px] bg-[#0b1422] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-cyan-400/50" required />
            ...
            {users.map((u) => (
              <tr key={u.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                ...
                {isAdmin && (
                  <td className="px-6 py-4">
                    {u.id !== currentUser?.id && (
                      <button onClick={() => handleDelete(u.id)} className="text-red-400 hover:text-red-300 text-sm\">حذف</button>
                    )}
                  </td>
                )}
              </tr>
